/*
    ...
    NOTE: I THINK THIS IS WRONG BECAUSE IM NOT SUPPOSED TO TRAVERSE THE QUEUE LIKE A NORMAL LINKED LIST
    E.G. (isFull(), enqueue())...
...
void read(LinkedList Queue) {
    ...
    for(LinkedList curr = Queue; curr != NULL; curr = curr->next) {
        printf("%d%s", curr->elem.data, (curr->next != NULL) ? ", " : ".\n"); // THIS IS OKAY CUZ WE'RE JUST DISPLAYING aha ha
    }
...
bool isFull(LinkedList Queue) {
    ...
    for(LinkedList curr = Queue; curr != NULL; curr = curr->next, i++) {} // TRAVERSING HERE IS WRONG I BELIEVE
...
void enqueue(LinkedList *Queue, int data) {
    if(!isFull(*Queue)) {
        ...
        for(trav = Queue; (*trav) != NULL; trav = &(*trav)->next) {} // ANOTHER TRAVERSAL
