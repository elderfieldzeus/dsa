...
// Function prototypes
int getValidChild(MinHeap pq, int left, int right);
...
int getValidChild(MinHeap pq, int left, int right) {
    return (right >= pq.size || pq.heap[left].duration < pq.heap[right].duration) ? left : right;
}

// Function to maintain the heap property by heapifying the subtree rooted at index i
void heapify(MinHeap* pq, int i) {
    // To do code logic here
    Task changeData = pq->heap[i];
    int parent = i;
    int left = i * 2 + 1;
    int right = i * 2 + 2;
    int child = getValidChild(*pq, left, right);
    
    // continually shift until parent becomes the right position for changeData
    while(
        child < pq->size
        && changeData.duration > pq->heap[child].duration
    ) {
        ...
        left = parent * 2 + 1;
        right = parent * 2 + 2;
        child = getValidChild(*pq, left, right);
    }
    
    pq->heap[parent] = changeData;
}
